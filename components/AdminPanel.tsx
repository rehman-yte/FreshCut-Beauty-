...
export const AdminPanel: React.FC<AdminPanelProps> = () => {
  ...
  const [stats, setStats] = useState({ revenue: 0, pending: 0, verif_pending: 0, verified_email: 0 });
  ...
  const fetchEverything = async () => {
    ...
    setStats({
      ...
      verified_email: us?.filter(u => u.role === 'customer' && u.otp_verified).length || 0
    });
  };
        ...
        <div className="flex-1 animate-fadeIn">
          {activeSection === 'overview' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="glass p-8 rounded-[2rem] border border-gold/10">
                <span className="text-[9px] font-black uppercase tracking-widest text-white/30 block mb-2">Email Verified</span>
                <span className="text-3xl font-serif font-black text-gold">{stats.verified_email}</span>
              </div>
              <div className="glass p-8 rounded-[2rem] border border-gold/10">
